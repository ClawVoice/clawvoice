...
function initPlugin(api: PluginAPI): void {
  ...
  const topLevelClawvoice =
    typeof rawCfg?.clawvoice === "object" && rawCfg.clawvoice !== null
      ? (rawCfg.clawvoice as Record<string, unknown>)
      : undefined;
  ...
  if (workspacePath) {
    const fsMemoryWriter = async (namespace: string, key: string, value: unknown): Promise<void> => {
  ...
  callService.setMemoryExtractor((callId, transcript) => {
    if (!config.autoExtractMemories) return;
    try {
  ...
  const legacyToolNames = new Set<string>();
  ...
  if (typeof toolsRegister === "function") {
    ...
    api.tools.register = ((def: Parameters<typeof origRegister>[0]) => {
      legacyToolNames.add(def.name);
      return origRegister(def);
    }) as typeof api.tools.register;
