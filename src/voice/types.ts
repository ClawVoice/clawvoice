...
export type VoiceAgentMessageResult =
  ...
  | { action: "disconnection"; record: DisconnectionRecord }
  | { action: "none" };
...
export type CodecNegotiationResult =
  | { ok: true; telephonyCodec: AudioCodec; voiceProviderCodec: AudioCodec; sampleRate: number }
...
export function negotiateCodec(
  ...
  const supportedTelephony: AudioCodec[] = ["mulaw"];
  const supportedVoice: AudioCodec[] = ["mulaw", "pcm16"];
