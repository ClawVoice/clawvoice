...
exports.runLiveConnectivityChecks = runLiveConnectivityChecks;
                ...
                try {
                    ...
                    if (phoneSid) {
                        ...
                        if (updateResp.ok) {
                            ...
                            if (errText)
                                clackLog.warn(errText.slice(0, 300));
                        }
                    }
                    else {
 ...
 */
async function runLiveConnectivityChecks(config, fetchFn = globalThis.fetch) {
    const checks = [];
    ...
    if (config.telephonyProvider === "twilio" && config.twilioAccountSid && config.twilioAuthToken) {
        try {
            ...
            const resp = await fetchFn(`https://api.twilio.com/2010-04-01/Accounts/${config.twilioAccountSid}.json`, { headers: { Authorization: `Basic ${auth}` }, signal: timeout(6000) });
